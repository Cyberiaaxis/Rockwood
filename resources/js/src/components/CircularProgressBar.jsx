...
export default function CircularProgressBar({ theme = 'light', ...props }) {
    const { className, size = 40, thickness = 3.6, value = 0, remain = 10, color, text = 100 } = props;

    const circumference = 2 * Math.PI * ((size - thickness) / 2);
    const strokeDashoffset = `${(((100 - value) / 100) * circumference).toFixed(3)}px`;

    const svgClass = classNames(color, {
        '-rotate-90': true,
    ...
    return (
        <div className="relative bg-white dark:bg-black flex items-center justify-center">
